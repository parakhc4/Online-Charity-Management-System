...
@RequestMapping("admin") // the annotation is used to map web requests to Spring Controller methods.
public class AdminController {

    @Autowired // Enabling annotation injection, we can use autowiring on properties, setters, and constructors.
    private AdminService airportService;

    @GetMapping("/getall") // annotation that acts as a shortcut for @RequestMapping.
    @ApiOperation("Fetch All Admins") // annotation to describe the endpoint and its response type
    public List<Admin> getEmployees() {
        return airportService.getAll();
    }

    @ApiOperation("Create new ADMIN") // annotation to describe the endpoint and its response type
    @PostMapping("/add") // Annotation for mapping HTTP POST requests onto specific handler methods.
    public ResponseEntity<Admin> createFlightRoute(@Valid @RequestBody Admin airport) {
        Admin cust = airportService.addAdmin(airport);
    ...
    public Admin fetchById(@PathVariable int id) {
